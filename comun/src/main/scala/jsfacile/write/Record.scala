...
trait Record {
	...
	def fail(failure: Exception): this.type;

	def failures: Seq[Exception];

	/** Appends a JSON representation of received instance of `T`, assuming an [[jsfacile.write.Appender]][T] exists in the implicit scope.
	 *
	 * This is the only method in the whole library with a by name implicit parameter of type [[jsfacile.write.Appender]][T]. All [[jsfacile.read.Parser]] summoning for a type `T` that is potentially recursive should use it. */
